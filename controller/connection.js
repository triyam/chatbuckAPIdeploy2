...
const sendConnectionRequest = async (req, res, next) => {
  try {
    ...
    const isConnectionExist = await Connection.findOne({
      $or: [{ user1: ObjectId(senderId) }, { user2: ObjectId(senderId) }],
    });
    ...
    const isAlreadySent = await ConnectionRequest.findOne({
      $and: [{ sender: ObjectId(senderId) }, { receiver: ObjectId(id) }],
    });
    if (isAlreadySent) {
...
const acceptConnectionRequest = async (req, res, next) => {
  try {
    ...
    const isConnectionExist = await Connection.findOne({
      $or: [{ user1: ObjectId(receiverId) }, { user2: ObjectId(receiverId) }],
    });
    if (isConnectionExist) {
      //Return connection already exist
      return res
    ...
    if (
      connectionRequest &&
      connectionRequest.receiver.toString() == receiverId.toString()
    ) {
...
const getAllConnections = async (req, res, next) => {
  try {
    ...
    return res
      .status(200)
      .json({
        message: `${connections.length} connections found`,
        success: true,
...
const ignoreConnectionRequest = async (req, res, next) => {
  try {
    ...
    if (!connectionRequest) {
      return res
        .status(400)
        .json({ message: "Invalid Connection", success: false, response: {} });
    }
    if (connectionRequest.receiver.toString() != req.user._id.toString()) {
      return res
        .status(400)
        .json({ message: "Invalid Request", success: false, response: {} });
    }
